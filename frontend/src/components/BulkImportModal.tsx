  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const result = await publishBulkProperties(data, user?.uid);

        setStatus("success");
        const inserted = result.count ?? 0;
        setRowCount(inserted);
        setMessage(`¡${inserted} de ${data.length} propiedades importadas correctamente!`);
        onSuccess?.();

        // Auto-cerrar modal tras 3 s para que el usuario vea el resultado real
